...
{
        // Calculate the padding for the last block
        int lastBlockLength = binaryInput.length() % 512;
        int paddingLength = 448 - lastBlockLength;
        if (paddingLength < 0)
        {
                paddingLength += 512;
        }
        ...
        paddedInput.append(paddingLength, '0');
...
void processBlock(string &binaryBlock, uint32_t &h0, uint32_t &h1, uint32_t &h2, uint32_t &h3, uint32_t &h4, uint32_t &h5, uint32_t &h6, uint32_t &h7)
{
        ...
        string paddedBlock = applyPadding(binaryBlock);
        string blockWithLength = appendLength(paddedBlock, binaryBlock.length());

        // Process the block
        uint32_t Words[64];
        formatMessageScheduel(blockWithLength, Words);
...
int main()
{
        ...
        */
        ...
        for (size_t i = 0; i < paddedInput.length(); i += 512)
        {
                string block = paddedInput.substr(i, 512);
